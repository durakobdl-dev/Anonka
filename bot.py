...
MOSCOW_TZ = timezone.utc        # можно поставить свой часовой пояс
...
async def notify_partner(user_id: int, text: str):
    """Отправить сообщение партнёру, если чат активен."""
...
async def start_cmd(message: types.Message):
    ...
    # Если был незавершённый лог (на всякий случай)
    # Создаём новый чат
...
async def stop_cmd(message: types.Message):
    ...
    if partner_id:
        ...
        # Лог уже заполнен, оставляем как есть
        await message.answer(MSG_CHAT_ENDED)
        await bot.send_message(partner_id, MSG_CHAT_ENDED)
    else:
        # Удаляем ожидающий инвайт, если есть
...
async def forward_text(message: types.Message):
    ...
    # Если в активном чате – пересылаем и пишем в лог
    partner_id = active_chats.get(user_id)
    if partner_id:
    ...
    # Если не в чате – ничего не делаем
...
async def show_logs(call: CallbackQuery):
    ...
    for log_key in chat_logs:
        # Извлекаем участников и время из ключа
...
async def toggle_ban(call: CallbackQuery):
    ...
    if uid in banned_users:
        ...
        # Если он в чате – разорвать связь
